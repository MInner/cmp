...
class IRTreeForestPrintVisitor : public ITreeVisitor
{
public:
	IRTreeForestPrintVisitor(std::ostream& out_): out(out_),
		clust(0), node(0) {}

	virtual ~IRTreeForestPrintVisitor() {}
	...
	int visit(const CodeFragment* codeFragment)
	{
		        ...
		        currentCodeFragment != nullptr; currentCodeFragment = currentCodeFragment->next )
		{
			...
			assert(currentCodeFragment->stmlist);
	...
	{
		subclust = 0;
		for(auto stump = stmlist;
		        stump != nullptr; stump = stump->tail )
		{
			out << "subgraph cluster" << clust << "_" << subclust++ << " {" << std::endl;
			out << "label=\"#" << subclust << "\";" << std::endl;
			stump->head->Accept(this); // because graphviz draws them in back order :(
			out << "}" << std::endl;
		}
	}
